...
updateAmbariConfigs() {
    updateResult=$($AMBARICONFIGS_PY $USERID $PASSWD $PORT http set $ACTIVEAMBARIHOST $CLUSTERNAME core-site  "fs.azure.account.key.$STORAGEACCOUNTNAME.blob.core.windows.net"  "$STORAGEACCOUNTKEY")
    ...
    updateResult=$($AMBARICONFIGS_PY $USERID $PASSWD $PORT http set $ACTIVEAMBARIHOST $CLUSTERNAME core-site "fs.azure.account.keyprovider.$STORAGEACCOUNTNAME.blob.core.windows.net"  "org.apache.hadoop.fs.azure.$KEYPROVIDER")
	if [[ $updateResult != *"Tag:version"* ]] && [[ $updateResult == *"[ERROR]"* ]]; then
		echo "[ERROR] Failed to update core-site. Exiting!"
		echo $updateResult
		exit 135
